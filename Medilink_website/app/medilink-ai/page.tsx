...
export default function MedilinkAI() {
  ...
  const [reportType, setReportType] = useState<"prescription" | "lab" | "other" | null>(null)
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (e.target.files && e.target.files[0]) {
      ...
      if (!validTypes.includes(file.type)) {
        setFileError("Invalid file type. Please upload an image (JPG, PNG, BMP, TIFF, WebP), PDF, or DOCX file.");
        return;
      }
  ...
  const extractMedications = (text: string) => {
    ...
    for (const section of sections) {
      if ((section.toLowerCase().includes("medication") || 
          ...
          section.match(/\d+\s*mg|\d+\s*mcg|\d+\s*ml|\d+\s*tablet|\d+\s*cap/i)) &&
    ...
    return medications.length > 0 ? medications : ["No medication details found"];
  }
  ...
  const extractInstructions = (text: string) => {
    ...
    for (const section of sections) {
      if ((section.toLowerCase().includes("instruction") || 
  ...
  const extractTestResults = (text: string) => {
    ...
    for (const section of sections) {
      if ((section.toLowerCase().includes("test") || 
           section.toLowerCase().includes("result") ||
           section.toLowerCase().includes("level") ||
           section.toLowerCase().includes("count") ||
           section.match(/\d+\s*mg\/dl|\d+\s*mmol|\d+\s*u\/l|\d+\s*g\/dl|\d+\s*mcg|\d+\s*pmol/i)) &&
          !section.toLowerCase().includes("summary") &&
          section.length > 10) {
  ...
  const extractAbnormalValues = (text: string) => {
    ...
    for (const section of sections) {
      if (section.toLowerCase().includes("abnormal") || 
  ...
  const extractSummary = (text: string) => {
    ...
    for (const section of sections) {
      if (section.toLowerCase().includes("summary") || section.toLowerCase().includes("impression")) {
        return section;
      }
    }
  ...
  const extractPrescriptionDetails = (analysis: string) => {
    ...
    const dateMatch = analysis.match(/(?:prescribed|date|written)(?:\s+on)?:\s*([A-Za-z]+ \d+,? \d{4}|\d{1,2}\/\d{1,2}\/\d{2,4}|\d{1,2}-\d{1,2}-\d{2,4})/i);
    if (dateMatch) details.date = dateMatch[1];
    
    const prescriberMatch = analysis.match(/(?:prescribed by|prescriber|doctor|physician|dr\.?):\s*([^,\n\.]+)/i);
    if (prescriberMatch) details.prescribedBy = prescriberMatch[1].trim();
    
    const durationMatch = analysis.match(/(?:duration|for|take for|period|course):\s*([^,\n\.]+\s+(?:days?|weeks?|months?))/i);
    if (durationMatch) details.duration = durationMatch[1].trim();
    
    const refillMatch = analysis.match(/(?:refills?|repeats?):\s*(\d+|zero|one|two|three|four|five|no\s+refills?)/i);
      ...
      <div className="bg-muted/30 border border-border rounded-lg overflow-hidden">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium">Medications ({medications.length})</span>
          </div>
                  ...
                  <div className="flex items-start">
                    ...
                    <div className="flex-1">
                      <div className="flex flex-wrap gap-2 mb-1">
                        {med.name && <h4 className="font-semibold">{med.name}</h4>}
                        {med.dosage && (
  ...
  const renderPrescriptionInstructions = () => {
    if (!reportData || reportType !== "prescription" || !reportData.general_instructions?.length) return null;
    ...
    return (
      <div className="bg-muted/30 border border-border rounded-lg overflow-hidden">
        ...
        {expandedSections.includes("instructions") && (
          <div className="p-4 border-t border-border">
            <ul className="space-y-3">
              {reportData.general_instructions.map((instruction: string, index: number) => (
                <li key={index} className="bg-background p-4 rounded-lg border border-border">
                  <div className="flex items-start">
  ...
  const renderPrescriptionWarnings = () => {
    if (!reportData || reportType !== "prescription" || !reportData.warnings?.length) return null;
          ...
          <div className="p-4 border-t border-border">
            <ul className="space-y-3">
              {reportData.warnings.map((warning: string, index: number) => (
                <li key={index} className="bg-orange-50 dark:bg-orange-900/10 p-4 rounded-lg border border-orange-200 dark:border-orange-800/20">
                  <div className="flex items-start">
      ...
      <div className="bg-muted/30 border border-border rounded-lg overflow-hidden">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium">Test Results ({testResults.length})</span>
          </div>
                  ...
                  <div key={index} className="bg-background p-4 rounded-lg border border-border">
                    <div className="flex items-start">
                      <div className={`h-6 w-6 rounded-full flex items-center justify-center mr-2 ${
                        ...
                        <FlaskConical className={`h-3 w-3 ${isAbnormal ? 'text-red-500' : 'text-amber-500'}`} />
                      </div>
                      <div className="flex-1">
                        <div className="flex justify-between items-start">
                          ...
                          {isAbnormal && (
                            <span className={`text-xs font-bold ${
                              test.status === "HIGH" ? "text-red-500" : "text-blue-500"
                            }`}>
                              {test.status}
                            </span>
                          )}
                        ...
                        {test.value && (
                          <div className="mt-1 flex items-center gap-2">
                            <span className={`text-sm font-medium rounded px-2 py-0.5 ${
                              isAbnormal 
            ...
            >
              <div className="p-4 border-b border-border">
                <div className="flex items-center gap-2">
                  ...
                  <X className="h-5 w-5 text-gray-900 cursor-pointer ml-auto" onClick={() => setSidebarOpen(false)} />
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-2">
                ...
                <div className="mt-4">
                  <h3 className="text-xs font-medium text-muted-foreground px-3 mb-1">RECENT CHATS</h3>
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="container mx-auto p-4 max-w-5xl">
              ...
              >
                ...
                <h1 className="text-3xl md:text-4xl font-bold text-primary mb-2 text-center">MediLink AI</h1>
                  ...
                  >
                    <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden">
                      <div className={`p-4 flex items-start gap-3 border-b border-border ${
                        reportType === "prescription" ? "bg-blue-500/10" : 
                        reportType === "lab" ? "bg-amber-500/10" : "bg-slate-500/10"
                      }`}>
                        <div className="mt-1">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-muted-foreground">
                            {reportType === "prescription" ? "Medications and dosage instructions" : 
                             reportType === "lab" ? "Test results and interpretations" : "Document content and summary"}
                          </p>
                          
                          {reportType === "prescription" && reportData.prescription_details && (
                            <div className="mt-3 grid grid-cols-2 gap-2">
                              {reportData.prescription_details.date && (
                                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                  <Calendar className="h-3.5 w-3.5" />
                                  <span>Date: {reportData.prescription_details.date}</span>
                                </div>
                              )}
                              {reportData.prescription_details.duration && (
                                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                  <Hourglass className="h-3.5 w-3.5" />
                                  <span>Duration: {reportData.prescription_details.duration}</span>
                                </div>
                              )}
                              {reportData.prescription_details.prescribed_by && (
                                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                  <User className="h-3.5 w-3.5" />
                                  <span>Prescribed by: {reportData.prescription_details.prescribed_by}</span>
                                </div>
                              )}
                              {reportData.prescription_details.refills && (
                                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                  <Clock className="h-3.5 w-3.5" />
                                  <span>Refills: {reportData.prescription_details.refills}</span>
                                </div>
                              )}
                            </div>
                          )}
                      ...
                      <div className="p-4 space-y-4">
                        ...
                        {reportType === "other" && (
                          <div className="bg-muted/30 border border-border rounded-lg p-4">
                            <p className="whitespace-pre-line">{reportData.content}</p>
                          </div>
                        )}
                      </div>
                    </div>
                ...
                <div className="bg-card rounded-xl shadow-sm border border-border p-5 flex flex-col min-h-[400px]">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {sessionId && (
                      <button 
                        ...
                        onClick={() => {/* Reset conversation logic */}}
                      >
                        New Chat
                      </button>
                    )}
                  ...
                  {!sessionId && !chatHistory.length && (
                    <div className="flex-1 flex flex-col items-center justify-center text-center p-6 text-muted-foreground">
                      ...
                      <h3 className="text-lg font-medium mb-2">Upload a medical document to begin</h3>
                      <p className="max-w-sm">
                        I can analyze prescriptions, lab reports, and other medical documents to help you understand them better.
                      </p>
                    </div>
                  )}
                  ...
                  <form onSubmit={handleSubmit} className="relative">
                    <input
                      type="text"
                      placeholder={sessionId ? "Ask questions about your medical report..." : "Upload a report to start chatting..."}
              ...
              {!sessionId && (
                ...
                >
                  <div className="bg-card rounded-xl shadow-sm border border-border p-5">
                    <h3 className="text-lg font-medium mb-3">How to use MediLink AI</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="p-4 bg-muted/30 rounded-lg border border-border">
                        ...
                        <h4 className="text-sm font-medium mb-2">1. Upload Document</h4>
                        <p className="text-sm text-muted-foreground">
                          Upload your medical report, prescription, or lab results for analysis.
                        </p>
                      </div>
                      ...
                      <div className="p-4 bg-muted/30 rounded-lg border border-border">
                        ...
                        <h4 className="text-sm font-medium mb-2">2. Review Analysis</h4>
                        <p className="text-sm text-muted-foreground">
                          Get an instant breakdown of your medical document with highlighted key information.
                        </p>
                      </div>
                      ...
                      <div className="p-4 bg-muted/30 rounded-lg border border-border">
                        ...
                        <h4 className="text-sm font-medium mb-2">3. Ask Questions</h4>
                        <p className="text-sm text-muted-foreground">
                          Chat with MediLink AI to get answers about your medical information in simple terms.
                        </p>
                      </div>
                    </div>
                  </div>
